...
# Default paths (if you deployed on a server and uploaded files to /mnt/data)
...
with st.form("predict"):
    ...
    for col in feature_names:
        # choose widget type based on df (if available) else default to text/number
        if df is not None and col in df.columns:
            ...
            if pd.api.types.is_numeric_dtype(series):
                ...
                inputs[col] = st.number_input(col, value=default_v, min_value=min_v, max_value=max_v, step=(max_v-min_v)/100 if max_v!=min_v else 1.0)
            else:
            ...
            # unknown column: allow free text but try number input first
            inputs[col] = st.text_input(col, value="")
...
if submitted:
    ...
    # attempt to convert numeric columns to numeric types when possible
    for c in X.columns:
        # if csv had numeric dtype, cast
    ...
    try:
        ...
        if hasattr(model, "predict_proba"):
            try:
                ...
                st.dataframe(pd.DataFrame(probs, index=[0]))
            except Exception:
                pass
    except Exception as e:
...
if df is not None:
    with st.expander("Show dataset (employee_clean_data.csv)"):
        ...
        st.download_button("Download CSV preview", data=csv, file_name="employee_clean_data_preview.csv", mime="text/csv")

st.write("---")
st.write("Notes:")
st.write("- Best practice: save your preprocessing + model as one pipeline so raw inputs work directly.")
st.write("- If model expects encoded columns, upload a CSV with the preprocessed columns matching training.")
